...
module EF_ADCS1008A_ahbl (
	...
	always @(posedge HCLK or negedge HRESETn) if(~HRESETn) TCTRL_REG <= 0; else if(ahbl_we & (last_HADDR==TCTRL_REG_ADDR)) TCTRL_REG <= HWDATA;
	always @(posedge HCLK or negedge HRESETn) if(~HRESETn) CHSEL_REG <= 0; else if(ahbl_we & (last_HADDR==CHSEL_REG_ADDR)) CHSEL_REG <= HWDATA;
	always @(posedge HCLK or negedge HRESETn) if(~HRESETn) CTRL_REG <= 0; else if(ahbl_we & (last_HADDR==CTRL_REG_ADDR)) CTRL_REG <= HWDATA;
	always @(posedge HCLK or negedge HRESETn) if(~HRESETn) SOC_REG <= 0; else if(ahbl_we & (last_HADDR==SOC_REG_ADDR)) SOC_REG <= HWDATA;
	always @(posedge HCLK or negedge HRESETn) if(~HRESETn) SEQCTRL0_REG <= 0; else if(ahbl_we & (last_HADDR==SEQCTRL0_REG_ADDR)) SEQCTRL0_REG <= HWDATA;
	always @(posedge HCLK or negedge HRESETn) if(~HRESETn) SEQCTRL1_REG <= 0; else if(ahbl_we & (last_HADDR==SEQCTRL1_REG_ADDR)) SEQCTRL1_REG <= HWDATA;
	always @(posedge HCLK or negedge HRESETn) if(~HRESETn) FIFOLEVEL_REG <= 0; else if(ahbl_we & (last_HADDR==FIFOLEVEL_REG_ADDR)) FIFOLEVEL_REG <= HWDATA;
	always @(posedge HCLK or negedge HRESETn) if(~HRESETn) IM_REG <= 0; else if(ahbl_we & (last_HADDR==IM_REG_ADDR)) IM_REG <= HWDATA;

	always @(posedge HCLK or negedge HRESETn) if(~HRESETn) ICR_REG <= 3'b0; else if(ahbl_we & (last_HADDR==ICR_REG_ADDR)) ICR_REG <= HWDATA; else ICR_REG <= 3'd0;
